...
-(NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    if ([_expandedFamilies containsObject:@(section)]) {
        return [[self fontsForFamily:[_families objectAtIndex:section]] count]+1;
    } else {
        if ([self fontsForFamily:[_families objectAtIndex:section]].count == 0) {
            return 0;
        } else {
            return 1;
        }
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row==0 && [[self fontsForFamily:family] count]>1) {
        ...
        [disclosureButton setImage:[UIImage imageNamed:@"Chevron"] forState:UIControlStateNormal];
        disclosureButton.frame = CGRectMake(0, 0, 44, 44);
        disclosureButton.imageView.contentMode = UIViewContentModeScaleAspectFit;
        disclosureButton.alpha = 0.5;
        [disclosureButton addTarget:self action:@selector(toggleDisclosure:) forControlEvents:UIControlEventTouchUpInside];
        if ([_expandedFamilies containsObject:@(indexPath.section)]) {
