...
- (id)initWithFilterType:(GPUImageShowcaseFilterType)newFilterType image:(UIImage *)image {
...
- (void)viewDidLoad
{
    ...
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"Apply" style:UIBarButtonItemStyleDone target:self action:@selector(done)];
}
...
- (void)setupFilter;
{
    picture = [[GPUImagePicture alloc] initWithImage:_image];
...
//            }];
    ...
    if (filterType == GPUIMAGE_FILECONFIG)
    {
        ...
        pipeline = [[GPUImageFilterPipeline alloc] initWithConfigurationFile:[[NSBundle mainBundle] URLForResource:@"SampleConfiguration" withExtension:@"plist"]
                                                                                               input:picture output:(GPUImageView*)self.view];
...
//            lineGenerator.crosshairWidth = 15.0;
        ...
        else if ( (filterType == GPUIMAGE_SPHEREREFRACTION) || (filterType == GPUIMAGE_GLASSSPHERE) )
        {
            ...
            [picture addTarget:gaussianBlur];
